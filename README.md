...
Transaction connectors attach to fixed sides: inputs enter the left and outputs leave the right, with separate attachment points ordered by ELK. **Connector appearance** in settings defaults to **Straight**; **Curved** and **Elbowed** are also available. Straight connections that return backward or would pass through objects use elbowed routes where needed. Curved keeps the curved appearance on every connector, including return connections, fee flows, and routed detours. Miro controls the final connector paths, so the local preview estimates its appearance. New connectors use the selected setting; choose **Sync and reorganize graph** to apply the calculated positions and connector appearance to an existing graph. Normal sync preserves manual positions and connector choices. To update an existing board from Plot Layouts, select Curved, generate a new layout, select it for the board, and choose **Sync and reorganize**. Previously saved layouts retain their captured appearance.
...
1. Open a saved investigation, or create one with a name and blockchain selection. **Liquid Network** is currently the only supported blockchain. Paste comma-separated transaction hashes, load their outputs, and select the relevant UTXOs grouped by transaction. Miro board setup belongs in **Miro boards**, after creation; the new-investigation form has no board field or starting-preferences panel.
...
3. Open **Plot Layouts** and choose a goal: **Full trace**, **Starter connections**, or **Paths to peg-outs**. Select a saved collection run, set any hop filter, adjust the layout settings, and generate the plot locally. Plotting fetches no new evidence or address statistics. Review its source coverage and layout.
4. From that saved plot, choose **Download ELK SVG** or **Sync with Miro**. Both use the same generated plot. **Sync with Miro** opens **Miro boards** with that plot selected; create or link a board for its goal, then choose **Sync to Miro** or **Sync and reorganize**. Every created or linked board has its own persistent block, with its Miro link, status, saved layout, layout picker, and sync controls. Each board keeps its own mapping; later plots can refresh the same board.
5. Use **History & downloads** to review earlier runs and retrieve saved SVGs, tables, layout reports, and supporting files. Saved layouts retain their captured appearance across restarts and later preference changes. Changed evidence or attribution inputs require regeneration; older plots without captured layout settings may need to be regenerated once.
...
The task tabs are **Collect data**, **Plot Layouts**, **Miro boards**, and **History & downloads**. The selected saved snapshot stays selected across tabs. **Collected data** shows **Hops collected**, the deepest recorded transaction hop in that snapshot, separately from its collection hop limit. Starting transactions are hop 0; this maximum does not imply every branch reached that depth. **Collect data** contains collection actions; use the tabs to move to plotting and the investigation heading to open settings. The main plotting workflow is fully offline; collection is the separate fetching phase. A plot is generated once for both its ELK SVG download and Miro publication. Older ELK and Mermaid tools can still fetch missing address counts before local rendering. Miro board creation and sync contact Miro and require its access token. Linking and listing boards are local actions.

The usual cycle is **Collect data → Plot Layouts → Download ELK SVG or sync with Miro**. Create or link each goal's board once. Normal sync preserves the positions of retained objects. If the existing positions leave no room, or you want the saved plot's arrangement, choose **Sync and reorganize**. Miro draws its own connector routes, so its appearance can differ from the local SVG. Obsolete generated objects are removed only after checks protect analyst changes and unrelated connections. Each board's status and history remain separate, while the underlying collection data is shared. You may also tidy the native shapes in Miro by hand.
...
**History & downloads**, with their original scope and resume controls. These legacy searches
