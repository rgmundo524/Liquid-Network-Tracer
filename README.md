...
Graph captions use `vin/vout number · amount asset`. For example, `vout 0 · ?? ??` means neither amount nor asset is available in the public data; `?? L-BTC` retains an explicitly identified asset with an unavailable amount. Recognized L-BTC values use whole units with exactly eight decimal places: `100000000` base units becomes `1.00000000 L-BTC`, and one satoshi becomes `0.00000001 L-BTC`. Explicit Bitcoin peg-in inputs use BTC with the same precision. Conversion uses integer arithmetic, including for values larger than JavaScript's safe integer range. Other or unidentified assets retain exact base units; their decimal precision is never guessed. Other asset IDs are shortened for display and retained in full in the evidence. Address circles show the address and any analyst attribution; the creating transaction hash and exact UTXO reference remain in the saved details instead of being repeated on the circle or input caption.

The terminal/browser output pickers, new graph previews, Miro captions, and new transaction CSV exports use this unit policy. Original API observations, saved trace values, and archived evidence tables keep their base-unit integers. To update an existing Miro graph, use **Sync to Miro** or **Sync and reorganize** with the saved run; unchanged generated captions refresh without retracing, while manually edited captions follow the existing conflict-preservation rules. Generate new previews and CSV exports to see converted amounts there. Earlier downloaded artifacts remain unchanged and keep their original units.
