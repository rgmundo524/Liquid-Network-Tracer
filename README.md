...
The search also compares arrangements that keep starting-transaction branches together and move transactions connecting different branches toward their facing edges. Shared addresses and merged flows are placed near the branches they connect. The first attempt retains the ordinary layout strategy; subsequent attempts alternate ordinary and branch-ordered arrangements when at least two branch interiors are present. The default 25 attempts includes both. A one-attempt search uses only the ordinary arrangement.
...
Candidate selection still prioritizes object collisions, arrows through objects, crossings, attachment conflicts and overlapping lines. After those checks, it prefers fewer interleaved branch interiors and connecting transactions closer to the relevant branch boundaries, before the remaining input-order and connection-length preferences. These are presentation preferences: heavily merged graphs or branches connecting several distant trees may not permit completely separate lanes. ELK recalculates all connector routes for the proposed arrangement; Miro still chooses its final bends.
