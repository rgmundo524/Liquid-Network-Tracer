...
If the cumulative graph exceeds 10,000 objects or 30,000 connections, or ELK reaches its time limit, the preview and Miro plan use the existing left-to-right dependency layout. They retain every displayed object and connection, fixed transaction input/output sides, and the separate fee row. The result is labelled **Dependency layout fallback** with its reason; it does not claim ELK crossing optimization. Miro sync and explicit reorganization can use this full layout. Reorganization still replaces managed positions, while ordinary sync retains its existing manual-position rules.
...
Mermaid preserves the graph's arrows, shapes, colors, dates, and compact quantity labels, but computes its own left-to-right layout. Fixed Miro connection sides, manually arranged positions, and the chronological fee row are not copied into a Mermaid-rendered chart. Above 1,000 objects or 2,000 connections, the command skips browser layout and creates a **Direct SVG fallback** from the dependency layout instead. It also falls back if a smaller graph reaches Mermaid's 120-second timeout. These are local scheduling thresholds, not Mermaid format limits. The fallback keeps every displayed object and connection, including transaction connection sides and the fee row, and is identified in the UI and saved preview. Both paths retain the complete `graph.mmd`, `graph.json`, and `mermaid-node-map.json`; the HTML is self-contained and can be viewed offline.

Direct SVG previews retain a separate safety limit of 50,000 combined objects and connections, with bounded coordinates and route points. If that limit is reached, the error reports it without omitting graph elements; Mermaid source remains saved and **Create CSV export** still exports the complete selected graph. A saved continuation is cumulative: reducing the next run's hop count will not shrink the existing graph. Select an earlier snapshot to preview less history. You can retry either preview from a saved run without fetching Blockstream data again.
...
Repeat sync checks all mapped items before writing. Existing-item reads and edits use up to **four concurrent requests**, with HTTPS connections reused where a proxy is not required. One shared limiter spaces request starts by 0.05 seconds for reads and 0.1 seconds for writes; workers do not each receive a separate allowance. This overlaps network latency while leaving room below Miro's published quota. New objects and fee removals remain sequential. Unchanged objects need no PATCH or per-item state rewrite, and changed fields on an object share one PATCH. Explicit reorganization still reasserts connector attachment points because Miro's returned coordinates cannot reliably distinguish automatic and fixed attachment modes.

Before sending edits, sync saves their intent. If an update fails, it stops dispatching additional requests and records successful responses already in flight before returning the error. Retry the same saved run: a fresh preflight reconciles those updates against the live board and preserves intervening manual content/style edits. Each acknowledged mutation is checkpointed locally. An interrupted creation still requires the explicit reconciliation described below.

Miro's [REST rate limits](https://developers.miro.com/reference/rate-limiting) are shared by each user/application combination, so another Liquid-trace instance can consume the same allowance. The limiter coordinates workers within one sync; it does not coordinate separate processes. Response quota headers and HTTP 429 responses can pause all workers. Bounded retries show their waiting state and respect the rate-reset header when `Retry-After` is absent. A requested wait above 30 seconds stops with an actionable terminal error; retry later using the same saved run and mapping. These improvements apply automatically to **Sync to Miro** and **Sync and reorganize graph**; no new settings or credentials are required.
