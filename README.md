...
Transaction inputs that consume an exact output already displayed from an earlier transaction enter first, from top to bottom on the transaction's left side. External context inputs follow. Where both groups are present, original `vin` numbers determine the order within each group; the labels, transaction records and CSV input order are not renumbered. Transactions with only one group retain ordinary ELK ordering. Merely reusing the same address does not give an unrelated UTXO priority. ELK's output order is retained. Explicit change objects keep their aligned rows; when several change inputs meet, one connector stays centered and the others enter at distinct positions with a final elbow. Shared address circles can have connections to several transactions, so their own vertical positions may differ from the order of the input attachments. Refresh older previews and choose **Sync and reorganize Miro graph** to apply the new ordering to an existing board.
...
The report compares the saved graph's baseline arrangement with the proposed ELK arrangement, **not the current live Miro board**. It estimates line crossings, object overlaps, and lines through unrelated objects. Counts marked `≥` are lower bounds when the comparison limit is reached. These collision counts exclude label boxes and Miro's automatic curves; zero estimated crossings does not guarantee a collision-free board. For smaller graphs ELK tries three deterministic alternatives: the original placement and two arrangements with the new branch priorities. Larger graphs use one arrangement with branch priorities. The comparison budget limits quality measurement work, never the accepted graph size or number of rendered objects. Optimization makes no external layout requests and uses no secrets.
