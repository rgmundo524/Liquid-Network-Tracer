...
Branch organization revision 2 adds presentation membership from the same verified UTXO lineage analysis that produces convergence markers. `graph.branch_structure` stores root transaction keys and node/edge memberships; it never feeds a shared address’s combined membership back into unrelated UTXOs. Unselected starting-output siblings and untraced context do not acquire lineage. The layout helper can give isolated context a placement preference near its transaction without changing this metadata. Filtered views restrict membership to their surviving nodes and edges.

For graphs with multiple branch interiors, the ELK scheduler alternates ordinary candidates with branch-ordered candidates by stable attempt index, starting with an ordinary candidate. Branch ordering groups interiors, places shared objects between participating branches and moves single-branch interaction transactions toward the facing edge. Exact UTXO matches carry that preference up the contributing paths; weaker inherited preferences keep their sibling subtrees together. Horizontal transaction-dependency partitions and object identities remain unchanged. The candidate’s connectors and captions are calculated by ELK, rather than retained after moving nodes outside the engine. New branch metrics measure per-column interleaving, boundary depth and interbranch travel after the existing collision/attachment gates and before input-policy and connection-length tie-breakers. Ordinary candidates remain eligible. The branch revision invalidates earlier reusable ELK previews while preserving their saved files and historical viewing.
