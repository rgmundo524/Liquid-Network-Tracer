...
Branch organization revision 3 retains presentation membership from the same verified UTXO lineage analysis that produces convergence markers. `graph.branch_structure` stores root transaction keys and node/edge memberships; it never feeds a shared address’s combined membership back into unrelated UTXOs. Unselected starting-output siblings and untraced context do not acquire lineage. The layout helper can give isolated context a placement preference near its transaction without changing this metadata. Filtered views restrict membership to their surviving nodes and edges.

Branch organization revision 3 adds `transaction_neighborhoods.py` to refine the seed-lineage order at every displayed fork, including graphs with only one starting transaction. Four forward/backward sweep pairs keep exclusive outputs and external inputs in contiguous blocks and use neighbor positions to order transactions and downstream activity. Shared addresses contribute all their neighbors without acquiring a false exclusive parent. Return connections, fee shapes and explicit hubs do not constrain forward ordering. The passes use sorting and rank inversion counts, with no recursion or graph-size cap. The preferred order also feeds named-group centering.
