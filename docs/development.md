...
In an interactive terminal, the command opens the Textual interface. Use the keyboard or mouse to select **New investigation**, **Continue investigation**, **Workspace defaults**, or **Exit**. The explicit launcher is:
...
Creating an investigation asks for a name, starting outputs, and an optional existing Miro board URL or ID. It inherits workspace defaults; edit the saved limits and graph preferences through **Investigation settings** after creation. New investigations use Live Liquid without a source selector. Enter known outputs as `HASH:NUMBER` in the multiline field, separated by whitespace or commas. `vout` is the numeric output index, not a word to enter: `:0` selects the first output and `:1` the second. To look up outputs, paste transaction hashes separated by commas into **Transaction hashes**, choose **Load outputs**, toggle the relevant rows across transactions with Enter, and choose **Use selected outputs**. The picker identifies each transaction and output separately, including outputs with the same index from different transactions. Nothing is preselected; applying a selection replaces the starting-output field.
...
**Rebuild on new board** publishes the selected saved run and current presentation settings to a new private board while preserving the old board and its mapping. It validates the graph and full fresh-item allowance before creation. The source-board ID pins the operation so stale dialogs and retries cannot silently create extra boards. A durable receipt retains the exact prepared plan and acknowledged destination. Incomplete operations offer **Resume board rebuild**; ambiguous board-creation responses require inspection and manual linking, without another automatic POST. Frames remain a separate action.

Board selection, run limits, and the **Include transaction fee flows** checkbox are saved with the case, so you do not need a board environment variable or a separate Nix file. Fees are excluded by default, including for older cases without that setting. The top-level **Workspace defaults** screen changes defaults for future investigations. Run dialogs expose only a hop allowance for that run, without rewriting saved defaults. Miro action dialogs use the linked board and saved sync budget; their settings shortcut opens **Investigation settings**. Existing cases retain their saved settings.

**ELK layout preview** uses the selected saved run and current display settings. Missing address transaction counts are fetched first, with Enterprise credentials when needed; ELK placement itself remains a local calculation. It opens a self-contained HTML/SVG view with estimated before/after quality counts and full graph details. The default Miro plan uses the same local layout engine. This is a preview of proposed geometry; it does not read the board's current positions.

**Mermaid chart** creates a local preview from the latest verified run, fetching missing address counts first and opening the secret provider only when Enterprise lookups require it. It is enabled once a run has been saved and does not require a Miro board. Rendering runs in the interface's background worker and opens the generated HTML in your browser; the result also shows its path. Each click creates `previews/<run-id>-mermaid-<id>/` inside the case, containing `graph.mmd`, genuine Mermaid-rendered `graph.svg`, a self-contained `graph.html`, full `graph.json` details, a node-ID mapping, and renderer configuration. The saved address mode and current fee choice apply; archived runs and Miro publication state are not modified.
...
| `liquid-live miro-rebuild-board --case CASE --source-board BOARD_ID --max-new-items 5000` | Prepares the updated graph, creates and links a new private board, then publishes it. Preserves the old board and resumes the same operation on retry. |
...
The browser supports new investigations with a name, blockchain selection (currently **Liquid Network** only), comma-separated transaction lookup, and grouped UTXO selection. Creation has no board field or starting-preferences panel. Task tabs are **Collect data**, **Plot Layouts**, **Miro boards**, and **History & downloads**. Collection contains fetching and continuation actions; plotting goals and layout controls belong in **Plot Layouts**. A saved plot supplies both **Download ELK SVG** and **Sync with Miro**; selecting an output does not generate a second plot. **Miro boards** creates and links destinations, matches saved layouts, and syncs them.

**Investigation settings** groups this case's Tracing, Miro sync budget, Colors, and Investigation data, including CSV imports, address review, and change outputs. **Workspace defaults** sets collection, plot layout, and Miro sync defaults copied into future cases. Both scopes persist; existing cases retain their settings. The seven layout controls appear in Workspace defaults for starting values and in **Plot Layouts** for the current case, but not in Investigation settings. **Plot Layouts** saves these preferences separately with the case through **Save layout settings**, or before **Generate** queues the plot. Unsaved layout drafts survive tab navigation and are isolated by case; saved preferences survive restarts. Fee flows, isolated context grouping, and separate branch hubs apply only to full-trace plots. Unsaved general settings survive opening the color editor; color edits save separately. Imports use Preview then Apply, with no additional review checkbox; plan fingerprints and changed-input invalidation still protect the exact batch. A historical run selection controls review and exports. Continuation follows the case's latest saved run so it extends the current lineage. See the [investigation workflow](investigation-workflow.md).

ELK and Mermaid **Download SVG** buttons, layout reports, and Mermaid source links live outside the sandboxed preview iframe. CSV tables each have a download button. The case-detail endpoint rediscovers the newest complete product of each kind for each saved run, checking the case/run identity and allowed nonsymlink files. Partial rendering attempts cannot hide a completed product. Downloads survive browser or server restarts. Newly saved Plot Layouts capture their display settings and remain usable after preference changes; changed evidence or attribution inputs require regeneration. Legacy products without captured settings can still show a settings mismatch and may need regeneration once. Artifacts remain in their original `previews/` and `exports/` directories, and immutable run archives are untouched.

Opening the browser, reading saved evidence, generating Plot Layouts, and exporting transaction CSVs do not resolve secrets. Older ELK and Mermaid preview actions can require credentials for automatic missing-address-count lookups. Live jobs invoke the same SecretSpec command, project manifest, provider, and profile as the terminal workflow. Proton Pass login or unlock prompts appear in the launching terminal. API credentials are never form fields or frontend build variables. Publishing to Miro requires a Miro token and network access.
...
Shape PATCHes complete before connector PATCHes, so attachment changes use settled endpoint geometry. Temporary PATCH failures have bounded read-before-replay recovery: a verified desired value can be acknowledged, while replay requires unchanged submitted fields and compatible identity/parent state. Ambiguous attachment modes and parent/frame moves require ordinary saved-state reconciliation. HTTP 400/422 is never blindly retried; diagnostics expose only known API field names, safe item references, recognized codes and correlation IDs. Completed live-sync ELK layouts are saved as compatible previews before publication, allowing a failed sync to reuse its layout without changing archived evidence or weakening preview invalidation.
...
After pulling changes into an already open devenv shell, run `liquid-layout-setup` or re-enter the shell. The ELK worker has no application graph-size or calculation deadline; local cancellation remains available. The saved `layout_attempts` setting defaults to 25 for every graph size and accepts integers from 1 to 1,000. `--layout-attempts N` overrides it for layout-preview, compact-preview and fresh miro-sync actions. Seeds run through separate Node processes in bounded parallel batches; each yields one or two attachment-order alternatives. The main thread scores results in seed order, preserving deterministic tie selection regardless of completion order. Only the current batch and winning full graph are retained, so memory use does not grow with the attempt count. The first seed runs alone with the full allowance and remains eligible to win. The Node worker reports its maximum resident set in MiB after serialization using `process.resourceUsage().maxRSS`; the adapter validates this bounded, numeric telemetry separately from the geometry response. Until a successful measurement is available, attempts stay serial. `LIQUID_ELK_WORKERS=auto` permits up to 64 workers, bounded by CPU affinity, Linux CPU quotas, remaining attempts and a share of at least twice the maximum observed peak RAM or 1,024 MiB, whichever is larger. Later worker measurements can reduce concurrency for subsequent batches. An explicit value from 1 to 64 is a concurrency ceiling. `LIQUID_RENDER_HEAP_MB` is the total ELK allowance, divided across workers; auto recalculates 90% of available memory between batches. Progress delivery occurs on the main thread. Cancellation and fatal errors stop and reap every active child. Parallel memory failures, including unconfirmed worker kills, retry the affected seeds once alone after the batch drains and keep the remaining search serial. Retry success counts as one successful seed. Search metadata records execution mode, peak worker count and memory retry count. Search revision 4 records the branch-first schedule and local neighborhood scoring; previews from earlier search revisions must be refreshed. The deterministic seed prefix starts with 1, 7 and 19. With branch ordering, attempt 1 uses flow-weighted placement and the balanced small-graph alternative runs at attempt 2. Later and large-graph attempts use flow-weighted placement. Named-group centering always uses flow-weighted placement. Increasing the count preserves the earlier candidates. Progress reports the current attempt and elapsed worker time, and the report saves search version, attempt count, seeds and selected seed. Candidate scoring considers object overlaps, local and estimated Miro connector collisions, endpoint order, shared attachment points and overlapping segments before named-group alignment, branch boundaries, local neighborhood order and transaction proximity. Connection travel and the traced-first preference break later ties. These are bounded estimates, not a guarantee of a global optimum or of Miro's undocumented routing. Truncated geometry counts are lower bounds and appear with `≥` in the local preview.

Search revision 4 retains revision 3’s isolation of nonzero worker-process exits with `ElkWorkerFailure`. The optimizer records that seed's fixed `failure_code`, retains any earlier winner, and continues the full requested seed sequence. `attempt_count` remains the requested count; `attempted_count`, `successful_count` and `failed_count` record actual outcomes. `failed_attempts` contains the failed attempt indexes, seeds and allowlisted error codes; `candidate_count` includes only returned candidates. Reports and progress visibly distinguish a best result with failed attempts from a completely successful search. Reuse accepts a completed search with failed attempts only when its counts, seed sequence and selected successful seed are internally consistent and all other presentation checks match. Cancellation, setup or input errors, malformed worker output and invalid returned geometry remain fatal even after an earlier candidate succeeds. If all workers fail, no new preview or Miro publication is produced.

The worker emits bounded structured diagnostics with fixed error codes and safe seed/profile/stage context, never raw engine messages or graph labels. Python classifies recognized heap, stack and engine errors without treating an unrecognized exit as a confirmed cause. Retaining successful candidates improves search resilience; it does not claim to fix the underlying ELK engine failure.

Branch organization revision 4 retains presentation membership from the same verified UTXO lineage analysis that produces convergence markers. `graph.branch_structure` stores root transaction keys and node/edge memberships; it never feeds a shared address’s combined membership back into unrelated UTXOs. Unselected starting-output siblings and untraced context do not acquire lineage. The layout helper can give isolated context a placement preference near its transaction without changing this metadata. Filtered views restrict membership to their surviving nodes and edges.

Branch organization revision 4 retains `transaction_neighborhoods.py` to refine the seed-lineage order at every displayed fork, including graphs with only one starting transaction. Four forward/backward sweep pairs keep exclusive outputs and external inputs in contiguous blocks and use neighbor positions to order transactions and downstream activity. Shared addresses contribute all their neighbors without acquiring a false exclusive parent. Return connections, fee shapes and explicit hubs do not constrain forward ordering. The passes use sorting and rank inversion counts, with no recursion or graph-size cap. The preferred order also feeds named-group centering.

The ELK scheduler alternates branch-ordered and unconstrained candidates by stable attempt index, starting with a branch-ordered candidate even for one attempt. Named-group centering keeps its own ordering on every attempt. Horizontal dependency partitions and object identities remain unchanged. ELK calculates the coordinates, connectors and captions. After collision, attachment, named-group and seed-boundary scores, candidates compare local flow-order inversions, sibling interleavings and transaction proximity. Flow-order inversions compare edges between the same pair of dependency columns; they are separate from geometric crossing estimates. Proximity weights the input and output sides equally so a large fan-out does not drown out the input side. Vertical chronology is not a placement constraint. Context compaction cannot worsen these neighborhood counts or transaction distance, and address compaction preserves dependency-column order. The branch and search revisions invalidate earlier reusable ELK previews while preserving their saved files and historical viewing.

Separate branch hubs use `hub_layout.py` to calculate temporary presentation columns. An exact input can restart depth only when its original vin, displayed input edge, outpoint and displayed parent output all identify the same selected Liquid hub. Another input from the same parent through a non-hub address remains a dependency. Iterative transaction ranking handles the retained dependencies; address columns follow their producers and consumers. ELK and the geometry-order metrics use this temporary view, while the saved original columns, transaction records, edge identities and input classifications stay unchanged. Hub graphs use source-first layering (`LONGEST_PATH_SOURCE`) so branches that end early do not push their spending transactions farther right than peers; node placement and route scoring still compare the existing candidates. The adapter exempts only these verified hub inputs from its original parent-order check. The hub remains one object at the left, incoming edges remain explicit return connections, and output branches expand right. Effective columns and hub roots are recorded in layout metadata.

ELK controls layering, connection-point ordering, and proposed routes. Transaction inputs use left-side ports and outputs use right-side ports. Fees are ordered by saved chain chronology in a row above the main flow. Straight is the default connector appearance; returns and obstructed connections may use elbowed exceptions in that mode. Curved and Elbowed retain the requested appearance for every edge while preserving routed paths and exception metadata. The routing-exception count records appearance overrides, so it is zero for Curved and Elbowed. Automatic ELK preview reuse rejects older Curved previews containing forced elbows; previously saved plot snapshots remain immutable and require a new layout to adopt the corrected appearance. Miro receives native shapes, endpoint positions, and connector appearance, but controls its own automatic route between those endpoints. Label overlaps and exact Miro curves are not scored.

The attachment corrections introduced in search revision 2 repair transaction output slots after explicit change-row alignment: non-fee sibling outputs occupy distinct lower slots sorted by their final destination attachment height. A subsequent endpoint-alignment pass can remove small horizontal jogs by moving a singleton address-side port along the circle, preserving all transaction ports and node coordinates. It excludes change continuations, fees, returns, busy address sides and long detours. Native-route and midpoint-elbow collision counts and attachment-order metrics must not worsen; incomplete measurements reject the optional adjustment. Changed routes discard stale ELK label coordinates. The pass runs before candidate scoring and again after accepted context movement is proposed. These corrections remain in revision 4; earlier saved ELK searches are not reused.
...
Incremental graph sync does not create, update, or remove export frames. A completed sync saves its exact framing plan alongside the board mapping. The separate `miro-frames` command, exposed as **Create / update Miro frames** in both interfaces, uses that completed snapshot and reads live shape positions, dimensions, and rotations. It does not recalculate layout or fetch blockchain data. Frame maintenance requires the selected graph to be fully synced and retains its display choices even if investigation defaults have since changed. Frame creation, updates, and obsolete-frame deletion keep the existing journals and explicit uncertain-POST recovery. Native children are detached only where necessary to change a generated frame, preserving their visible positions. Older interrupted graph syncs finish once through ordinary sync before using the separate frame action.
...
The user-facing export is now a single `transactions.csv`: one row per displayed transaction input/output, using full hashes, UTC block times, exact base-unit amounts, attributions, peg-out request amounts, IN/OUT direction, and zero-based vin/vout indexes. Fee visibility follows the selected graph. Unknown amounts and USD valuations stay blank. Internal graph tables and raw evidence remain in the immutable run archive, but are not copied into this export. See [Transaction CSV](transaction-csv.md) for all columns, units, and connection-only downloads.
