...
The user-facing export is now a single `transactions.csv`: one row per displayed transaction input/output, using full hashes, UTC block times, exact L-BTC/BTC amounts with eight decimal places (other or unidentified assets retain base units), attributions, peg-out request amounts, IN/OUT direction, and zero-based vin/vout indexes. Fee visibility follows the selected graph. Unknown amounts and USD valuations stay blank. Internal graph tables and raw evidence remain in the immutable run archive, but are not copied into this export. See [Transaction CSV](transaction-csv.md) for all columns, units, and connection-only downloads.
