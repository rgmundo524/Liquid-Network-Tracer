...
The browser supports new Live Liquid investigations, comma-separated transaction lookup and grouped UTXO selection, bounded data collection and continuation, offline plotting goals, saved-run review, SVG and CSV downloads, and multiple managed Miro boards. Task tabs are **Collect data**, **Plot views**, **Miro boards**, and **History & downloads**. A saved plot supplies both **Download ELK SVG** and **Sync with Miro**; selecting an output does not generate a second plot. **Investigation settings** groups Tracing, Graph layout, Miro, Colors, and Investigation data, including CSV imports, address review, and change outputs. **Workspace defaults** applies to future cases. Unsaved settings survive opening the color editor; color edits save separately. Imports use Preview then Apply, with no additional review checkbox; plan fingerprints and changed-input invalidation still protect the exact batch. A historical run selection controls review and exports. Continuation follows the case's latest saved run so it extends the current lineage. See the [investigation workflow](investigation-workflow.md).
