...
export function addressImportPanel(caseId: string, busy: boolean): string {
  ...
  <p class="address-note">CSV, JSON, or plain address lists. A plain list defaults to suspected confidence with tracing stopped. CSV/JSON accepts Address, Name, confidence, stop_tracing, hop_limit, source, and notes. CSV columns can be in any order; unrelated columns such as Duplicate count are ignored. Supported fields still require valid values, and JSON rejects unsupported fields. hop_limit is blank for no local cap or a nonnegative number of additional transaction hops. Stop tracing takes priority. Confidence and stopping are independent. Up to 5,000 rows / 512 KiB. This action is local and does not start a trace.</p>
