...
export function addressImportPanel(caseId: string, busy: boolean): string {
  ...
  <p class="address-note">CSV, JSON, or plain address lists. A plain list becomes suspected-service stops. CSV/JSON can specify service names, confidence, sources, notes, and labels that do not stop tracing. Up to 5,000 rows / 512 KiB. This action is local and does not start a trace.</p>
  ...
  <div class="table-wrap"><table><thead><tr><th>Row</th><th>Address</th><th>Action</th><th>Name</th><th>Classification</th><th>Confidence</th><th>Stop</th><th>Evidence / previous entry</th></tr></thead><tbody>${review.changes.slice(draft.offset, draft.offset + 100).map(entry => `<tr><td>${entry.row}</td><td class="mono">${esc(entry.rule.address)}</td><td>${esc(entry.action)}</td><td>${esc(entry.rule.name)}</td><td>${esc(entry.rule.classification)}</td><td>${esc(entry.rule.confidence)}</td><td>${entry.rule.enabled && entry.rule.stop_tracing ? 'Yes' : 'No'}</td><td><details><summary>Review fields</summary><pre>${esc(JSON.stringify({incoming: entry.rule, existing: entry.previous}, null, 2))}</pre></details></td></tr>`).join('')}</tbody></table></div>
