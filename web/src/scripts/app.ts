...
function resultBanner(action: string, result: Result): string {
  if (["csv", "mermaid"].includes(action)) return "";
  const titles: Record<string, string> = {
    trace: "Run saved",
    "miro-preview": "Miro change preview",
    "miro-sync": "Miro sync finished",
    "miro-organize": "Miro organization finished",
...
function openActionDialog(action: string): void {
  ...
  const titles: Record<string, string> = {
    ...
    "miro-organize": "Organize the Miro graph",
  ...
  const descriptions: Record<string, string> = {
    ...
    "miro-organize":
      "Apply the generated left-to-right layout and transaction connection sides to tracked board items. This moves items you may have arranged by hand.",
    "miro-create":
...
async function dispatch(action: string, element?: HTMLElement): Promise<void> {
  ...
  if (["mermaid", "csv", "miro-preview"].includes(action))
