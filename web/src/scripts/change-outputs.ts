...
function importPanel(locked: boolean): string {
  ...
  <p class="address-note">Upload or paste CSV or JSON with Txid, ChangeVout, and optional Notes. Each transaction can have one change output. Headers are case-insensitive. CSV columns can be in any order; unrelated columns such as Duplicate count are ignored. Supported fields still require valid values, and JSON rejects unsupported fields. A blank ChangeVout clears an existing annotation only when replacement is enabled. Up to 512 KiB.</p>
